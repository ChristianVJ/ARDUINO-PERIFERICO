...
/* Programa para reproducir una música mediante un buzzer 
y para o reproducir la canción mediante un botón*/
...
#define buzzer 3 // Pin del buzzer
...
void melodia(){      
     ...
     int size = sizeof(starwars_melody) / sizeof(int); // Tamaño de la melodia
     
     for (int thisNote = 0; thisNote < size; thisNote++) {
     
       int noteDuration = 1000/starwars_tempo[thisNote];
       buzz(buzzer, starwars_melody[thisNote],noteDuration);

       int pauseBetweenNotes = noteDuration *0.50; // Tiempo entre notas
       delay(pauseBetweenNotes); // Aplicamos el retraso
       buzz(buzzer, 0,noteDuration); // Para la musica
...
void buzz(int targetPin, long frequency, long length) {
  ...
  int state=digitalRead(boton); //Detecta si el botón ha pasado a otro estado
  ...
  while (digitalRead (boton));     // Pulsar boton
